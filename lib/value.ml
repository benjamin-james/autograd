(* Autograd primitives similarly implemented to micrograd *)
...
let relu a =
  make_node (if a.vals > 0.0 then a.vals else 0.0) (RELU (a, a.vals > 0.0))

let parents = function
  | ADD (a, b) | MUL (a, b) | SUB (a, b) | DIV (a, b) -> [ a; b ]
  | EXP a
  | LOG a
  | NEG a
  | SIN a
...
let backward root =
  ...
  List.iter
    (fun n ->
      ...
      | EXP a -> a.grad <- a.grad +. (n.vals *. n.grad)
      | LOG a -> a.grad <- a.grad +. (n.grad /. a.vals)
      | NEG a -> a.grad <- a.grad -. n.grad
      | SIN a -> a.grad <- a.grad +. (n.grad *. Stdlib.cos a.vals)
      | COS a -> a.grad <- a.grad -. (n.grad *. Stdlib.sin a.vals)
      | TANH a -> a.grad <- a.grad +. (n.grad *. (1. -. (n.vals *. n.vals)))
      | SIGMOID a -> a.grad <- a.grad +. (n.grad *. n.vals *. (1. -. n.vals))
      | POW (a, k) ->
          a.grad <-
            (a.grad
            +.
            if k <> 0 then
              float_of_int k *. (a.vals ** float_of_int (k - 1)) *. n.grad
            else 0.0)
      | SQRT a -> a.grad <- a.grad +. (n.grad /. (2. *. n.vals))
